...
export default function EditProduct() {
    ...
    const onSubmit = async (e) => {
        ...
        try {
            await axios.put(`http://localhost:8080/product/${id}`, product);
            setSuccess("Товар успешно обновлен! Перенаправляем...");

            setTimeout(() => {
                navigate("/");
            }, 1500);
        } catch (err) {
            setError(err.response?.data?.message || "Ошибка при обновлении товара");
        } finally {
            setLoading(false);
        }
    };
            ...
            <Container maxWidth="sm" sx={{ padding: '0 16px' }}>
                ...
                >
                    ...
                    >
                        ...
                        >
                            {product.image ? "Изменить изображение" : "Загрузить изображение"}
                            <input
                                type="file"
                                hidden
